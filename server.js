...
app.get('/fetchdepartmentss', function(req, res) {
    pg.connect(process.env.DATABASE_URL, function (err, conn, done) {
        // watch for any connect issues
        if (err) console.log(err);
		conn.query(
			'SELECT sfid, Name FROM salesforce.Department__c',
			function(err, result) {
				if (err) {
					res.status(400).json({error: err.message});
				}
...
app.post('/fetchsubdepartmentss', function(req, res) {
    pg.connect(process.env.DATABASE_URL, function (err, conn, done) {
        ...
        if (err) console.log(err);
		conn.query(
			'SELECT sfid, Name FROM salesforce.Sub_Department__c WHERE LOWER(Department__c) = LOWER($1)',
			[req.body.seldept.trim()],
			function(err, result) {
				if (err) {
					res.status(400).json({error: err.message});
				}
...
app.post('/fetchdeptname', function(req, res) {
    pg.connect(process.env.DATABASE_URL, function (err, conn, done) {
        ...
        if (err) console.log(err);
		conn.query(
			'SELECT sfid, Name FROM salesforce.Department__c WHERE LOWER(sfid) = LOWER($1)',
			[req.body.seldept.trim()],
			function(err, result) {
				if (err) {
					res.status(400).json({error: err.message});
...
app.post('/fetchsubdeptname', function(req, res) {
    pg.connect(process.env.DATABASE_URL, function (err, conn, done) {
        // watch for any connect issues
        if (err) console.log(err);
		conn.query(
			'SELECT sfid, Name FROM salesforce.Sub_Department__c WHERE LOWER(sfid) = LOWER($1)',
			[req.body.selsubdept.trim()],
			function(err, result) {
				if (err) {
					res.status(400).json({error: err.message});
				}
